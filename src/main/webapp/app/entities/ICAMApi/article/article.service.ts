...
export class ArticleService {
  ...
  protected convertDateFromClient(article: IArticle): IArticle {
    const copy: IArticle = Object.assign({}, article, {
      ...
      importDate: article.importDate && article.importDate.isValid() ? article.importDate.format(DATE_FORMAT) : undefined
    });
    return copy;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((article: IArticle) => {
        ...
        article.importDate = article.importDate ? moment(article.importDate) : undefined;
      });
    }
    return res;
  }
